...
def Mue_downward_IEMs(f, angle_t, angle_i, epsr, delta, corr_len, spectrum='exp', x=1.5, shadow_flag=True):
    """Fung 1994 IEM model for single downward scattering.
    ...
    k2 = 2 * np.pi * f * np.sqrt(mu2 * epsr) / sci_const.speed_of_light
    k = k1
    kdel = k * delta # real?
    kdel2 = kdel * kdel
    ...
    for i in range(4): # i for qp
        ...
        for j in range(4): # j for rs
            ...
            Iqpn = (nksz + nkz)**nv * fqp * np.exp(-delta**2 * np.real(ksz) * np.real(kz)) \
                + 0.5 * (nksz**nv * Fqp + nkz**nv * Fqps)
            Irsn = (nksz + nkz)**nv * frs * np.exp(-delta**2 * np.real(ksz) * np.real(kz)) \
