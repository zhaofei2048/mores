""" Integral Equation Model (IEM) for rough surface scattering
...
def Mue_upward_IEMs(f, angle_s, angle_i, epsr, delta, corr_len, spectrum='exp', x=1.5, shadow_flag=True):
    """Fung 1994 IEM model for single upward scattering.
    ...
    k1 = 2 * np.pi * f * np.sqrt(mur * 1) / sci_const.speed_of_light
    k2 = 2 * np.pi * f * np.sqrt(mur * epsr) / sci_const.speed_of_light
    k = k1
    # eta_r = np.sqrt(mur / epsr) # relative wave impedance
    ...
    #--------------------------
    cssp = np.sqrt(1 + cs * css - s * ss * csfs)/np.sqrt(2)
    ...
    rvh = rvhi
    u = -kx; v = -ky
    Fqp_ = [IEMFvv(u, v, rv, vpara), IEMFvh(u, v, rvh, vpara), IEMFhv(u, v, rvh, vpara), IEMFhh(u, v, rh, vpara)]
    u = -ksx; v = -ksy
    Fqps_ = [IEMFvv(u, v, rv, vpara), IEMFvh(u, v, rvh, vpara), IEMFhv(u, v, rvh, vpara), IEMFhh(u, v, rh, vpara)]
    ...
    for i in range(4): # i for qp
        ...
        for j in range(4): # j for rs
            ...
            Iqpn = (nksz + nkz)**nv * fqp * np.exp(-delta**2 * ksz * kz) \
                + 0.5 * (nksz**nv * Fqp + nkz**nv * Fqps)
            Irsn = (nksz + nkz)**nv * frs * np.exp(-delta**2 * ksz * kz) \
                + 0.5 * (nksz**nv * Frs + nkz**nv * Frss)

            Iqprsn = Iqpn * np.conj(Irsn)
            # print(f"dtype: {kdel2_n.dtype}")
            Sqprs[i, j] = np.sum(Iqprsn * kdel2_n * Wn)
    
    C0 = k**2 / (8 * np.pi) * np.exp(-delta**2 * (ksz**2 + kz**2))
...
#===============================================
# Complementary field coefficient function for IEM
def IEMFvv(u, v, rv, vpara):
    k1, k2, kx, ky, kz, ksx, ksy, ksz, v_, h_, vs_, hs_, mur, er, eta_ist  = vpara

    rvp = (1 + rv)
    rvm = (1 - rv)
    qi = np.sqrt(k1**2 - u**2 - v**2)
    qt = np.sqrt(k2**2 - u**2 - v**2)
    n1_, n2_, g_ = calc_n1_n2_g(kx, ky, kz, ksx, ksy, ksz, u, v, 1, 0)

    C = Ccoeff(k1, vs_, hs_, n1_, n2_, v_, h_, g_)

    F = -(rvm / qi - rvp * mur / qt) * rvp * C[0] \
        + (rvm / qi - rvp / qt) * rvm * C[1] \
        + (rvm / qi - rvp / er / qt) * rvp * C[2] \
        + eta_ist * (\
        (rvp / qi - rvm * er / qt) * rvm * C[3] \
        + (rvp / qi - rvm / qt) * rvp * C[4] \
        + (rvp / qi - rvm / mur / qt) * rvm * C[5] \
        )
    
    return F


def IEMFhh(u, v, rh, vpara):
    k1, k2, kx, ky, kz, ksx, ksy, ksz, v_, h_, vs_, hs_, mur, er, eta_ist  = vpara
    ...
    F = -(rhp / qi - rhm * mur / qt) * rhm * C[3] \
        -(rhp / qi - rhm / qt) * rhp * C[4] \
        -(rhp / qi - rhm / er / qt) * rhm * C[5] \
        + eta_ist * (\
        (rhm / qi - rhp * er / qt) * rhp * C[0] \
        - (rhm / qi - rhp / qt) * rhm * C[1] \
        - (rhm / qi - rhp / mur / qt) * rhp * C[2] \
        )
    
    return F


def IEMFvh(u, v, rvh, vpara):
    k1, k2, kx, ky, kz, ksx, ksy, ksz, v_, h_, vs_, hs_, mur, er, eta_ist  = vpara
    ...
    F1 = rm / qi * rp * B[3] + rm / qi * rm * B[4] + rm / qi * rp * B[5] \
        + eta_ist * (rp / qi * rm * B[0] - rp / qi * rp * B[1] - rp / qi * rm * B[2])
    F2 = -rp * mur / qt * rp * B[3] - rp / qt * rm * B[4] - rp / qt / er * rp * B[5] \
        + eta_ist * (-rm * er / qt * rm * B[0] + rm / qt * rp * B[1] + rm / qt / mur * rm * B[2])
...
def IEMFhv(u, v, rvh, vpara):
    ...
    F1 = rm / qi * rp * B[0] - rm / qi * rm * B[1] - rm / qi * rp * B[2] \
        + eta_ist * (rp / qi * rm * B[3] + rp / qi * rp * B[4] + rp / qi * rm * B[5])
    F2 = -rp * mur / qt * rp * B[0] + rp / qt * rm * B[1] + rp / qt / er * rp * B[2] \
        + eta_ist * (-rm * er / qt * rm * B[3] - rm / qt * rp * B[4] - rm / qt / mur * rm * B[5])
