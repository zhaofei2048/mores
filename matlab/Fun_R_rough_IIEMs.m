...
function [Ms, Coeff_test, sca_test] = Fun_R_rough_IIEMs(f, angle_s, angle_i, epsilon_1, epsilon_2, sp, SpR, x, shadow_flag)
%% check inputs
if ~exist('x', 'var')
    x = 1.5;
end
...
for n = 1:Ts
    ...
    % 应该是n次方吧（其实不是，因为在算系数C时，多乘了(ksz±qi)啥的，所以这里需要少乘以一个）
%     Ivv(n) = (kz + ksz).^n .* fvv .* exp(-sig^2 .* kz .* ksz) + ...
%         0.25*(Fvvupi .*(ksz-qi).^(n) .*exp(-sig^2 .*(qi.^2 - qi.*(ksz-kz)))+ ...
%         Fvvdni .*(ksz+qi).^(n) .*exp(-sig^2 .*(qi.^2 + qi.*(ksz-kz)))+ ...
%         Fvvups .*(kz+qs).^(n) .*exp(-sig^2 .*(qs.^2 - qs.*(ksz-kz)))+ ...
%         Fvvdns .*(kz-qs).^(n) .*exp(-sig^2 .*(qs.^2 + qs.*(ksz-kz))));
% 
%     
%     Ihh(n) = (kz + ksz).^n .* fhh .* exp(-sig^2 .* kz .* ksz) + ...
%         0.25*(Fhhupi .*(ksz-qi).^(n) .*exp(-sig^2 .*(qi.^2 - qi.*(ksz-kz)))+ ...
%         Fhhdni .*(ksz+qi).^(n) .*exp(-sig^2 .*(qi.^2 + qi.*(ksz-kz)))+ ...
%         Fhhups .*(kz+qs).^(n) .*exp(-sig^2 .*(qs.^2 - qs.*(ksz-kz)))+ ...
%         Fhhdns .*(kz-qs).^(n) .*exp(-sig^2 .*(qs.^2 + qs.*(ksz-kz))));
end
